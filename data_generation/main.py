...
from utils.api_call import togther_api_few_shots
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(path, "prev_stop.txt")):
        with open(os.path.join(path, "prev_stop.txt"), "r") as file:
            prev_stop = file.read()
    ...
    for i in pbar:
        try:
            ...
            output0 = togther_api_few_shots(ex[0]["prompt"], option = ex[0]["options"][0], few_shots = few_shots[0], verbose = False)
            output1 = togther_api_few_shots(ex[0]["prompt"], option = ex[0]["options"][1], few_shots = few_shots[0], verbose = False)
            output2 = togther_api_few_shots(ex[0]["prompt"], option = ex[0]["options"][2], few_shots = few_shots[0], verbose = False)
            output3 = togther_api_few_shots(ex[0]["prompt"], option = ex[0]["options"][3], few_shots = few_shots[0], verbose = False)
            ...
            if i%10 == 0 and i > 0:
                write_data(new_data, os.path.join(path,"generations", "few_shot", "_" + str(prev_stop) + ".json"))
                print ("Writing")
                new_data = []
                prev_stop = i+1
                write_data(str(prev_stop), r"C:\Users\J C SINGLA\PycharmProjects\olab\medhalt\prev_stop.txt")
        except:
            print ("\nSleeping")
            time.sleep(3)
