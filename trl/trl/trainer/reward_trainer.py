...
#             )
#         warnings.warn(inputs["input_ids_chosen"].device, inputs["input_ids_rejected"].device)
        
#         rewards_chosen = model(
#             input_ids=inputs["input_ids_chosen"].to("cuda"),
#             attention_mask=inputs["attention_mask_chosen"].to("cuda"),
#         )[0]
#         rewards_rejected = model(
#             input_ids=inputs["input_ids_rejected"].to("cuda"),
#             attention_mask=inputs["attention_mask_rejected"].to("cuda"),
#         )[0]
...
class RewardTrainer(Trainer):
    ...
    def compute_loss(
        ...
        rewards_rejected0 = model(
            input_ids=inputs["input_ids_rejected0"],
            attention_mask=inputs["attention_mask_rejected0"],
        )[0]
        rewards_rejected1 = model(
            input_ids=inputs["input_ids_rejected1"],
            attention_mask=inputs["attention_mask_rejected1"],
        )[0]
        rewards_rejected2 = model(
            input_ids=inputs["input_ids_rejected2"],
            attention_mask=inputs["attention_mask_rejected2"],
        )[0]
        # calculate loss, optionally modulate with margin
        if "margin" in inputs:
            loss = -nn.functional.logsigmoid(rewards_chosen - rewards_rejected - inputs["margin"]).mean()
        else:
            loss = -nn.functional.logsigmoid(rewards_chosen - rewards_rejected0 - rewards_rejected1 - rewards_rejected2).mean()

        if return_outputs:
            return loss, {
                "rewards_chosen": rewards_chosen,
                "rewards_rejected0": rewards_rejected0,
                "rewards_rejected1": rewards_rejected1,
                "rewards_rejected2": rewards_rejected2,
