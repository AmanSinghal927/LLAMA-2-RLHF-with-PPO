...
#             if (
#                 "input_ids_chosen" not in feature
#                 or "input_ids_rejected0" not in feature
#                 or "input_ids_rejected1" not in feature
#                 or "input_ids_rejected2" not in feature
#             ):
#                 raise ValueError(
#                     "The features should include `input_ids_chosen`, `input_ids_rejected0`, `input_ids_rejected1` and `input_ids_rejected2`"
#                 )
            ...
            features_rejected0.append(
                {
                    "input_ids": feature["input_ids_rejected0"],
                    "attention_mask": feature["attention_mask_rejected0"],
                })
            features_rejected1.append(
                {
                    "input_ids": feature["input_ids_rejected1"],
                    "attention_mask": feature["attention_mask_rejected1"],
                })
            features_rejected2.append(
                {
                    "input_ids": feature["input_ids_rejected2"],
                    "attention_mask": feature["attention_mask_rejected2"],
                }
            )
            if has_margin:
                margin.append(feature["margin"])
        ...
        batch_rejected0 = self.tokenizer.pad(
            features_rejected0,
            padding=self.padding,
            max_length=self.max_length,
            pad_to_multiple_of=self.pad_to_multiple_of,
        ...
        batch_rejected1 = self.tokenizer.pad(
            features_rejected1,
            padding=self.padding,
            max_length=self.max_length,
            pad_to_multiple_of=self.pad_to_multiple_of,
        ...
        batch_rejected2 = self.tokenizer.pad(
            features_rejected2,
            padding=self.padding,
            max_length=self.max_length,
            pad_to_multiple_of=self.pad_to_multiple_of,
        ...
        batch = {
            "input_ids_chosen": batch_chosen["input_ids"],
            "attention_mask_chosen": batch_chosen["attention_mask"],
            "input_ids_rejected0": batch_rejected0["input_ids"],
            "attention_mask_rejected0": batch_rejected0["attention_mask"],
            "input_ids_rejected1": batch_rejected1["input_ids"],
            "attention_mask_rejected1": batch_rejected1["attention_mask"],
            "input_ids_rejected2": batch_rejected2["input_ids"],
            "attention_mask_rejected2": batch_rejected2["attention_mask"],
            "return_loss": True,
        }
        if has_margin:
            margin = torch.tensor(margin, dtype=torch.float)
